...
export const ServiceSetlistModal: React.FC<Props> = ({ service, isOpen, onClose }) => {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
        <div className="bg-white rounded-3xl p-6 max-w-md w-full text-center shadow-xl border border-slate-200">
          ...
          <p className="text-xs text-slate-500 mt-2">
            Solo el <strong>Administrador</strong> o la <strong>Voz Director programada para este culto</strong> pueden gestionar las canciones.
          </p>
          <button
            onClick={onClose}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-2xl max-w-3xl w-full max-h-[96vh] sm:max-h-[92vh] flex flex-col overflow-hidden my-auto">
        
        {/* Header Planning Center Style */}
        <div className="p-3 sm:p-5 border-b border-slate-100 flex items-center justify-between gap-2.5 bg-slate-50/70">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-emerald-600 text-white flex items-center justify-center font-bold shadow-sm shadow-emerald-600/30 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-2 text-slate-400 hover:text-slate-700 hover:bg-white rounded-xl transition-colors flex-shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-3 sm:p-5 overflow-y-auto space-y-4 sm:space-y-5 flex-1">
          ...
          <div>
            ...
            {songs.length === 0 ? (
              <div className="p-4 sm:p-6 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                <Music className="w-7 h-7 text-slate-400 mx-auto mb-1.5" />
                <p className="text-xs font-bold text-slate-700">Aún no hay canciones en este culto</p>
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Agrega abajo la primera canción seleccionándola del banco o creando una nueva rápida.
                </p>
              </div>
            ) : (
              <div className="space-y-2 max-h-[360px] overflow-y-auto pr-1">
                {songs.map((song, index) => {
                  ...
                  return (
                    <div 
                      key={song.id}
                      className="p-3 sm:p-3.5 rounded-2xl border border-slate-200 bg-white hover:border-emerald-300 transition-all shadow-2xs"
                    >
                      <div className="flex items-start justify-between gap-2.5">
                        <div className="flex items-start gap-2.5 min-w-0 flex-1">
                          {/* Número */}
                          <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg sm:rounded-xl bg-slate-100 text-slate-700 flex items-center justify-center text-[11px] sm:text-xs font-black flex-shrink-0 mt-0.5">
                            #{index + 1}
                          </div>
          ...
          {!editingSongId ? (
            <div className="p-3 sm:p-4 border border-slate-200 bg-slate-50/90 rounded-2xl space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                ...
                <div className="grid grid-cols-2 gap-1 p-1 bg-slate-200/70 rounded-xl w-full sm:w-auto">
                  <button
                    type="button"
              ...
              {addMode === 'bank' && (
                <div className="space-y-2.5 bg-white p-3 sm:p-4 rounded-2xl border border-slate-200/90 shadow-2xs">
                  {/* Buscador */}
                  <div className="relative">
                    <Search className="w-3.5 h-3.5 text-slate-400 absolute left-3 top-2.5 pointer-events-none" />
                    <input
                      type="text"
                      placeholder="Buscar por título o autor en el banco..."
                  ...
                  {songBank.length === 0 ? (
                    ...
                    <div className="max-h-64 sm:max-h-72 overflow-y-auto space-y-2 pr-1">
                      {songBank
                        .filter(b => {
                        ...
                        .map(item => {
                          ...
                          return (
                            <div
                              key={item.id}
                              className="p-2.5 sm:p-3 rounded-xl bg-white border border-slate-200/80 hover:border-emerald-300 hover:shadow-2xs transition-all space-y-1.5"
                            >
                              {/* FILA 1: Título completo (sin recortar) + Tono + Botón Agregar */}
                              <div className="flex items-start justify-between gap-2.5">
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    ...
                                    {item.defaultKey && (
                                      <span className="text-[10px] font-black text-emerald-800 bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 rounded-md flex-shrink-0">
                                        {item.defaultKey}
                                      </span>
                                    )}
                                  </div>
                                ...
                                <button
                                  ...
                                  className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 transition-all active:scale-95 shadow-2xs flex-shrink-0 ${
                                    isAlreadyInService
                                ...
                                >
                                  ...
                                  <span>{isAlreadyInService ? 'Añadir +' : 'Agregar'}</span>
                                </button>
                              </div>

                              {/* FILA 2: Veces cantadas + BPM / Pista / Video */}
                              <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-100/80 text-[10px]">
                                <span className={`font-bold px-2 py-0.5 rounded-full inline-flex items-center gap-1 ${
                                  times === 0 
                                    ? 'bg-slate-100 text-slate-600' 
                                    : times >= 4 
                                    ? 'bg-amber-100 text-amber-900 border border-amber-200' 
                                ...
                                <div className="flex items-center gap-2 text-slate-400">
                                  {item.bpm && <span className="font-mono">{item.bpm} BPM</span>}
                                  {item.audioUrl && <span className="text-teal-700 font-bold">🎧 Pista</span>}
                                  {item.youtubeUrl && <span className="text-rose-600 font-bold">▶ Video</span>}
                                </div>
                              </div>
                            </div>
                          );
        ...
        {/* Footer Actions */}
        <div className="p-3 sm:p-4 border-t border-slate-100 bg-slate-50/80 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
          <button
            type="button"
            onClick={handleCopyPublicLink}
            className="text-[11px] text-slate-600 hover:text-emerald-700 font-bold flex items-center gap-1.5 transition-colors truncate self-start sm:self-auto"
          >
            <Globe className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
            <span className="truncate">Link: {publicRepertoireUrl.slice(0, 32)}...</span>
          </button>
          ...
          <div className="grid grid-cols-2 sm:flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
              className="py-2.5 px-4 text-xs font-bold text-slate-600 hover:text-slate-900 bg-white border border-slate-200 rounded-xl transition-colors text-center"
            >
              Cancelar
            </button>
            ...
            <button
              ...
              onClick={handleSave}
              disabled={isSaving}
              className="py-2.5 px-5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black rounded-xl transition-all shadow-sm shadow-emerald-600/30 flex items-center justify-center gap-1.5 disabled:opacity-50 text-center"
            >
              {isSaving ? (
                <span>Guardando...</span>
              ) : (
